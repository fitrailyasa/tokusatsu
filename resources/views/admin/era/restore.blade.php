...
<button role="button" class="btn btn-sm m-1 btn-dark" data-bs-toggle="modal"
    data-bs-target=".formRestore{{ $era->id }}"><i class="fas fa-undo"></i><span class="d-none d-sm-inline">
        {{ __('Restore') }}</span></button>

<!-- Modal -->
<div class="modal fade formRestore{{ $era->id }}" tabindex="-1" role="dialog" aria-hidden="">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{ route('admin.era.restore', $era->id) }}" method="POST">
                    @method('PUT')
                    @csrf
