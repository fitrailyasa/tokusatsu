...
<button role="button" class="btn btn-sm m-1 btn-danger" data-bs-toggle="modal"
    data-bs-target=".formDelete{{ $geojson->id }}"><i class="fas fa-skull"></i><span class="d-none d-sm-inline">
        {{ __('Delete') }}</span></button>

<!-- Modal -->
<div class="modal fade formDelete{{ $geojson->id }}" tabindex="-1" role="dialog" aria-hidden="">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{ route('admin.geojson.destroy', $geojson->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <x-button.close />
