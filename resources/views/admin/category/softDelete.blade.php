...
<button role="button" class="btn btn-sm m-1 btn-danger" data-bs-toggle="modal"
    data-bs-target=".formSoftDelete{{ $category->id }}"><i class="fas fa-trash"></i><span class="d-none d-sm-inline">
        {{ __('Delete') }}</span></button>

<!-- Modal -->
<div class="modal fade formSoftDelete{{ $category->id }}" tabindex="-1" role="dialog" aria-hidden="">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{ route('admin.category.softDelete', $category->id) }}" method="POST">
                    @method('DELETE')
                    @csrf
