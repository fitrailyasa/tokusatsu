...
<x-slot name="formCreate">
    @can('create:' . $permission)
        @if (Route::has('admin.' . $permission . '.store'))
...
<x-slot name="import">
    @can('import:' . $permission)
        @if (Route::has('admin.' . $permission . '.import'))
            ...
            <!-- Modal -->
            <div class="modal fade formImport" tabindex="-1" role="dialog" aria-labelledby="modalFormLabel"
                aria-hidden="true">
...
<x-slot name="exportExcel">
    @can('export:' . $permission)
        @if (Route::has('admin.' . $permission . '.exportExcel'))
...
<x-slot name="exportPDF">
    @can('export:' . $permission)
        @if (Route::has('admin.' . $permission . '.exportPDF'))
...
<x-slot name="softDeleteAll">
    @can('soft-delete-all:' . $permission)
        @if (Route::has('admin.' . $permission . '.softDeleteAll'))
...
<x-slot name="restoreAll">
    @can('restore-all:' . $permission)
        @if (Route::has('admin.' . $permission . '.restoreAll'))
...
<x-slot name="deleteAll">
    @can('delete-all:' . $permission)
        @if (Route::has('admin.' . $permission . '.destroyAll'))
            <x-table-modal modalMethod="DELETE" route="{{ route('admin.' . $permission . '.destroyAll') }}"
                ...
                buttonText="Delete All" buttonIcon="fa-skull-crossbones" buttonColor="btn-danger"
                modalType="hardDeleteAll" />
        @endif
    @endcan
</x-slot>
